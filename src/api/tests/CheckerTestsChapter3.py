...
class CheckerTestsChapter3(BaseCheckerTest):
    ...
    """
    Test curly brace spacing checks, Check 3.6.1-3.6.4
    TODO: This method is not complete
    """
    def test_curly_brace_spacing_checker_success(self):
        self.base_test_check_success(
            f"{self.CHAPTER_NUM}/BadCurlyBraceSpacing.java",
            [
                f"[ERROR] " + self.TEST_FILE_PATH + ":1:35: '{' at column 35 should have line break after. [LeftCurly]",
                "[ERROR] " + self.TEST_FILE_PATH + ":3:35: '{' at column 35 should have line break after. [LeftCurly]",
                f"[ERROR] {self.TEST_FILE_PATH}:3:37: 'ctor def' child has incorrect indentation level 4, expected level should be 8. [Indentation]",
                "[ERROR] " + self.TEST_FILE_PATH + ":8:9: '}' is not followed by whitespace. [WhitespaceAround]",
                "[ERROR] " + self.TEST_FILE_PATH + ":8:10: '}' is not preceded with whitespace. [WhitespaceAround]",
                f"[ERROR] {self.TEST_FILE_PATH}:12:7: 'METHOD_DEF' should be separated from previous line. [EmptyLineSeparator]"
            ]
        )
